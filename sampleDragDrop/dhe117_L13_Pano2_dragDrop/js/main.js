...
$(document).ready(function(){
	...
	initTargetVals.initOp = $('.dragTarget').css('opacity');
...
*/
    ...
    $.ajax({
        ...
        success: function (xml) {
            ...
            if($(xml).find('customEndMsg')){

				customEndMsg = $(xml).find('customEndMsg').text().trim();
			}
	...
	$('.dragger').draggable({
		...
		stop:function(){
			...
			if(isAccepted){
				...
				$('#t'+parsedtID).velocity({opacity:initTargetVals.initOp}, {duration:150});
    ...
    for(var i=0;i<tArray.length;i++){
    	...
    	$('#'+$(tArray[i]).attr('id')).droppable({
			...
			drop:function(event, ui){
				...
				parsedtID = parseInt(dropID.replace('t',''),10);

				if(parsedtID===parsedID){
			...
			over:function(event, ui){
				...
				$(this).velocity({opacity:1},{duration:150});
			},
			out:function(event, ui){

				$(this).velocity({opacity:initTargetVals.initOp}, {duration:150});
			}
        });
...
function getFeedback(){
	...
	if(customEndMsg.length>0){

		$('#feedback').append('<div id="customEndMsg" class="customEndMsg" aria-labelledby="customEndMsg" tabindex=0>'+customEndMsg+'</div> ');
	}
...
function resetThis(){
	...
	$('#customEndMsg').remove();
	$('.dragTarget').velocity({opacity:initTargetVals.initOp},{duration:0});
